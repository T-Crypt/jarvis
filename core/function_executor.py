...
class FunctionExecutor:
    ...
    def _control_rgb_lighting(self, params: Dict) -> Dict:
        """Apply an RGB lighting effect via SignalRGB URL scheme (PowerShell).

        Uses: start signalrgb://effect/apply/<EffectSlug>
        No API key or Pro subscription required — just SignalRGB running.
        """
        import subprocess

        effect_name = params.get("effect_name", "")
        if not effect_name:
            return {"success": False, "message": "No effect name specified", "data": None}
        ...
        if not hasattr(self, 'signalrgb_client') or not self.signalrgb_client:
            try:
                ...
                return {"success": False, "message": f"SignalRGB client unavailable: {e}", "data": None}

        success = self.signalrgb_client.apply_effect(effect_name)
        if success:
            return {
                "success": True,
                "message": f"RGB effect changed to {effect_name}",
                "data": {"effect_name": effect_name}
            }
        return {
            "success": False,
            "message": f"Failed to apply RGB effect '{effect_name}'. Is SignalRGB running?",
            "data": None
        }
